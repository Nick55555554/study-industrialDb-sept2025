...
export class AttackRepository {
    ...
    async update(id: string, attackData: UpdateAttackRequest): Promise<Attack | null> {
        try {
    ...
    async updateWithTargets(attackId: string, attackData: UpdateAttackRequest, targetsData?: UpdateTargetRequest[]): Promise<AttackWithTargets | null> {
        return await db.transaction(async (trx) => {
            try {
                ...
                if (targetsData && targetsData.length > 0) {
                    updatedTargets = await this.updateTargetsForAttack(trx, attackId, targetsData);
                } else {
                    // Получаем текущие цели
                    updatedTargets = await trx('targets').where({ attack_id: attackId });
                }
    ...
    private async updateTargetsForAttack(trx: any, attackId: string, targetsData: UpdateTargetRequest[]): Promise<Target[]> {
        ...
        await db.transaction(async (trx) => {
            if (!attacksTableExists) {
                await trx.schema.createTable('ddos_attacks', (table) => {
                    ...
                    table.specificType('affected_ports', 'integer[]').defaultTo('{}');
                    table.specificType('mitigation_strategies', 'text[]').defaultTo('{}');
